...
{
    public class SlashComboFeint : StateModifierBase
    {
        public override string BindState => "";

        private const string DecisionStateName = "SlashCombo Feint Decision";

        // Les deux états qui peuvent maintenant déclencher le follow-up
        private readonly string[] entryStates = new[]
        {
                "Slash Antic G"    // hit case
        };

        public SlashComboFeint(PlayMakerFSM fsm, PlayMakerFSM stunFsm, SethWrapper wrapper)
        ...
        public override void OnCreateModifier()
        {
            ...
            var dec = new FsmState(fsm.Fsm)
            {
                ...
                Transitions = new[]
                {
                    new FsmTransition(){ FsmEvent = FsmEvent.GetFsmEvent("NOTHING"), ToState = "Slash Combo 1",ToFsmState = fsm.Fsm.GetState("Slash Combo 1") },
