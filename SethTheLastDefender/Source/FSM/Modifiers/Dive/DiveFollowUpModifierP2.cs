...
{
    public class DiveFollowUpModifierP2 : StateModifierBase
    {
        public override string BindState => "LOL";

        private const string DecisionStateName = "P2 Dive Follow Up Decision";

        // États qui déclenchent le follow-up
        private readonly string[] entryStates = new[] { "Dive Land" };

        public DiveFollowUpModifierP2(PlayMakerFSM fsm, PlayMakerFSM stunFsm, SethWrapper wrapper)
            ...
            var dec = new FsmState(fsm.Fsm)
            {
                ...
                Actions = new FsmStateAction[]
                {
                    new WeightedRandomEventAction()
                    {
                        ...
                        weights = new float[] { 0.2f, 0.21f, 0.03f , 0.21f, 0.35f}
