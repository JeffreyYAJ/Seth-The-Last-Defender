...
{
    public class DiveFollowUpModifierP3 : StateModifierBase
    {
        public override string BindState => "LOL";

        private const string DecisionStateName = "P3 Dive Follow Up Decision";

        // États qui déclenchent le follow-up
        private readonly string[] entryStates = new[] { "Dive Land" };

        public DiveFollowUpModifierP3(PlayMakerFSM fsm, PlayMakerFSM stunFsm, SethWrapper wrapper)
            ...
            var dec = new FsmState(fsm.Fsm)
            {
                ...
                Actions = new FsmStateAction[]
                {
                    new WeightedRandomEventAction()
                    {
                        ...
                        weights = new float[] { 0.16f, 0.16f, 0.16f, 0.5f, 0.02f}
                    }
                },
                Transitions = new[]
                {
