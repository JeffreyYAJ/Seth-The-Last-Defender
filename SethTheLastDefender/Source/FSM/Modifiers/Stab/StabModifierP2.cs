...
{
    public class StabModifierP2 : StateModifierBase
    {
        public override string BindState => "LOL";

        private const string DecisionStateName = "P2 Stab Follow Up Decision";

        // États qui déclenchent le follow-up
        private readonly string[] entryStates = new[] { "Stab Recover" };

        public StabModifierP2(PlayMakerFSM fsm, PlayMakerFSM stunFsm, SethWrapper wrapper)
            ...
            var dec = new FsmState(fsm.Fsm)
            {
                ...
                Actions = new FsmStateAction[]
                {
                    new WeightedRandomEventAction()
                    {
                        ...
                        weights = new [] { 0.2f, 0.35f, 0.21f,0.21f, 0.03f }
