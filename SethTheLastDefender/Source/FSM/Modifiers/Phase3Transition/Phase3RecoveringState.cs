...
namespace SethPrime
{
    public class Phase3RecoveringState : StateModifierBase
    {

        public override string BindState => "Phase 3 Recovering State";

        public Phase3RecoveringState(PlayMakerFSM fsm, PlayMakerFSM stunFsm, SethWrapper wrapper)
        ...
        private void CreateBindState()
        {
            ...
            var state = new FsmState(fsm.Fsm)
            {
                ...
                Actions = new FsmStateAction[]
                {
                    ...
                    new Wait() { time = 3f, finishEvent = FsmEvent.GetFsmEvent("FINISHED") },
                ...
                Transitions = new[]
                {
                    new FsmTransition()
                    {
                        ...
                        ToState = "Phase 3 Recovering Music",
                        ToFsmState = fsm.Fsm.GetState("Phase 3 Recovering Music")
                    }
                }
            };

            fsm.Fsm.States = fsm.Fsm.States.Append(state).ToArray();
            SethPrimeMain.Log.LogInfo("[Phase3RecoveringState] State créé et ajouté au Control FSM : " + BindState);
        }
    }
}
