...
{
    public class GShieldThrowFollowUpModifier : StateModifierBase
    {
        public override string BindState => "LOL";

        private const string DecisionStateName = "P1 GShield Throw Follow Up Decision";

        // États qui déclenchent le follow-up
        private readonly string[] entryStates = new[] { "Catch Ground" };

        public GShieldThrowFollowUpModifier(PlayMakerFSM fsm, PlayMakerFSM stunFsm, SethWrapper wrapper)
            ...
            var dec = new FsmState(fsm.Fsm)
            {
                ...
                Actions = new FsmStateAction[]
                {
                    new WeightedRandomEventAction()
                    {
                        ...
                        weights = new float[] { 0.77f, 0.2f, 0.03f }
                    }
                },
                Transitions = new[]
                {
