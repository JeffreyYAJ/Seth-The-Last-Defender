...
{
    public class GShieldThrowFollowUpModifierP2 : StateModifierBase
    {
        public override string BindState => "LOL";

        private const string DecisionStateName = "P2 GShield Throw Follow Up Decision";

        // États qui déclenchent le follow-up
        private readonly string[] entryStates = new[] { "Catch Ground" };

        public GShieldThrowFollowUpModifierP2(PlayMakerFSM fsm, PlayMakerFSM stunFsm, SethWrapper wrapper)
            ...
            var dec = new FsmState(fsm.Fsm)
            {
                ...
                Actions = new FsmStateAction[]
                {
                    new WeightedRandomEventAction()
                    {
                        ...
                        weights = new float[] { 0.62f, 0.35f, 0.03f }
                    }
                },
                Transitions = new[]
                {
