...
namespace SethPrime
{
    public class RecoverCombo : StateModifierBase
    {

        public override string BindState => "Recover Combo";

        public RecoverCombo(PlayMakerFSM fsm, PlayMakerFSM stunFsm, SethWrapper wrapper)
            ...
            var state = new FsmState(fsm.Fsm)
            {
                ...
                Transitions = new[]
                {
                    new FsmTransition()
                    {
                        ...
                        ToState = "Recover Combo 1",
                        ToFsmState = fsm.Fsm.GetState("Recover Combo 1")
                    }
                }
            };

            fsm.Fsm.States = fsm.Fsm.States.Append(state).ToArray();
            SethPrimeMain.Log.LogInfo("[Recover Combo] State créé et ajouté au Control FSM : " + BindState);
        }
    }
}
